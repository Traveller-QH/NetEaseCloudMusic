...
const playSong = async (song) => {
  // 如果当前在"TA 的热门作品"分类，直接使用热门歌曲列表
  if (activeTab.value === 'home') {
    const songIndex = hotSongs.value.findIndex(s => String(s.id) === String(song.id))
    if (songIndex >= 0 && hotSongs.value.length > 0) {
      musicStore.setPlaylist(hotSongs.value, song.id)
  ...
  // 如果在"单曲"分类或没有其他数据，使用全量加载逻辑
  const preloadData = musicStore.getPreloadData()
  
  // 如果已经有预加载数据且已加载完成，直接使用
  if (preloadData.playlistId === artistId.value && 
  ...
  // 在后台开始全量加载 (不阻塞 UI)
  try {
    await musicStore.loadAllSongsForPlaylist(artistId.value, async (size, offset) => {
      return await getArtistSongs(artistId.value, 'hot', size, offset)
    }, {
      pageSize: 100,
      maxSongs: 1000,
      delayMs: 50,
